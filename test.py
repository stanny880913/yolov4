...
def test(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(
            path, imgsz, batch_size, 64, opt, pad=0.5, rect=True)[0]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                x = pred.clone()
                x[:, :4] = scale_coords(
                    img[si].shape[1:], x[:, :4], shapes[si][0], shapes[si][1])  # to original
            ...
            if plots and len(wandb_images) < log_imgs:
                ...
                boxes = {"predictions": {
                    "box_data": box_data, "class_labels": names}}
    ...
    if plots and wandb:
        ...
        wandb.log({"Validation": [wandb.Image(str(x), caption=x.name)
