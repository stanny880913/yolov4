...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if pretrained:
        ...
        state_dict = {k: v for k, v in ckpt['model'].items() if model.state_dict()[
            k].numel() == v.numel()}
    ...
    if wandb and wandb.run is None:
        ...
        wandb_run = wandb.init(config=opt, resume="allow",
                               project='YOLOv4' if opt.project == 'runs/train' else Path(
                                   opt.project).stem,
                               name=save_dir.stem,
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            if plots:
                ...
                if wandb:
                    wandb.log({"Labels": [wandb.Image(
                        str(x), caption=x.name) for x in save_dir.glob('*labels*.png')]})
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    plot_images(images=imgs, targets=targets,
                                paths=paths, fname=f)
                    # if tb_writer:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str,
                        default='yolov4.weights', help='initial weights path')
